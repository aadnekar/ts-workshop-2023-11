 ...
 */
namespace reverse {
  type Reverse<Tuple> = Tuple extends [infer First, ...infer Rest] ? [...Reverse<Rest>, First] : Tuple;
 ...
 */
namespace split {
  type Split<Str, Separator extends string> = Str extends `${infer Element}${Separator}${infer Rest}` ? [Element, ...Split<Rest, Separator>] : [Str];
