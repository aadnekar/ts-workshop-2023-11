import { Equal, Expect, Compute } from "../common"
 ...
 */
namespace tupleHead {
  type SafeHead<Tuple extends any[], Default> = Tuple extends [infer First, ...any] ? First : Default;
  ...
  type res3 = SafeHead<["BoogieMan", "Suger", 53], 43>
  type test3 = Expect<Equal<res3, "BoogieMan">>
}
 ...
 */
namespace last {
  type Last<Tuple extends any[]> = Tuple extends [...any, infer Last] ? Last : never;
...
namespace getFirstWord {
  type GetFirstWord<Str> = Str extends `${infer FirstWord} ${string}` ? FirstWord : Str;

  type res1 = GetFirstWord<"Frontend Summit 🎉">;
  type test1 = Expect<Equal<res1, "Frontend">>;

  type res2 = GetFirstWord<"🔥">;
  type test2 = Expect<Equal<Compute<res2>, "🔥">>;
 ...
 */
namespace getGroups {
  type GetGroups<Query> = Query extends `${string} by {${infer Groups}}` ? Groups : undefined;
  ...
  type groups3 = GetGroups<"perf.render.duration{component:univiz} by ">;
  type test3 = Expect<Equal<groups3, undefined>>;
}
